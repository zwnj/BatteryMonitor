# BatteryMonitor3 プロジェクト整理メモ

リファクタリング前に、現在の構成と責務を確認するためのメモです。
2026-06-30 時点でコードを読んだ内容をもとにしています。

## 概要

BatteryMonitor3 は Windows のシステムトレイに常駐する WPF アプリです。
...
`%LOCALAPPDATA%\BatteryMonitor3\settings.json` に設定を保存します。
...
- `README.md`、コメント、UI 表示文字列の多くが文字化けしています。
- ただし `dotnet build BatteryMonitor3.sln` は成功しており、コンパイル上は壊れていません。
